  ...
  return (
    <section id="portfolio" className="relative py-20 sm:py-24 bg-slate-50/70 overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-wrap justify-center items-center gap-2 mb-10">
          {categories.map((cat) => (
            <button
              ...
              className={`px-3.5 py-1.5 rounded-lg text-xs sm:text-sm font-semibold transition-colors border ${
        ...
        {/* Projects Grid - Clean instant render without jitter */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredProjects.map((project) => (
            <div
              key={project._id || project.title}
              className="rounded-2xl p-5 sm:p-6 bg-white border border-slate-200 hover:border-slate-300 hover:shadow-md transition-all duration-200 flex flex-col justify-between shadow-xs"
            >
              <div>
                {/* Image Container */}
