...
import { ArrowRight, Sparkles, Shield, Clock, Award, BarChart3 } from "lucide-react";
...
export function Hero({ heroData }: HeroProps) {
  ...
  return (
    <section className="relative pt-32 pb-20 sm:pt-36 sm:pb-24 overflow-hidden bg-slate-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        
        {/* Clean top badge */}
        <div className="flex justify-center mb-6">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border border-blue-200 bg-blue-50/80 text-blue-700 text-xs font-semibold shadow-xs">
        ...
        <div className="text-center max-w-4xl mx-auto mb-10">
          ...
          <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-3">
            ...
            <a
              href="#services"
              className="w-full sm:w-auto px-6 py-3 rounded-lg font-semibold text-slate-700 bg-white hover:bg-slate-100 border border-slate-300 shadow-xs transition-colors flex items-center justify-center gap-2 text-sm sm:text-base"
            >
        ...
        <div className="mt-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 max-w-5xl mx-auto">
          {highlights.map((item: any, idx: number) => (
            <div
              key={item.title || idx}
              className="p-4 rounded-xl bg-white border border-slate-200 shadow-xs text-left"
            >
